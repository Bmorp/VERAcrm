...
app.post("/api/sync-sheet", sheetsLimiter, async (req, res) => {
  ...
  Object.keys(entry).forEach((key) => {
    ...
    // Alphanumeric Integrity: Only clean phone if it's purely numeric (standard lead)
    // If it contains letters (Quick Lead name), preserve it.
    if (key === "phone" && val && /^\d+$/.test(val.toString().trim())) {
        val = val.toString().replace(/\D/g, "").slice(-10);
    }
    
    params.set(key, val);
  });

  // Use App-generated ID for audit continuity
  const transactionId = entry.transactionId || entry.id || ((entry.phone || "WALK") + "-" + Date.now());
